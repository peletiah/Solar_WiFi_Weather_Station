...
void setup() {
  ...
  if (! uv.begin()) {
    Serial.println("Didnt' find GY-1145");
  }

  Serial.println("GY-1145 is ok!");
...
//*******************************************************************************
  ...
  char _measured_uv_index[8];                                // Buffer big enough for 7-character float
  dtostrf(measured_uv_index, 3, 1, _measured_uv_index);               // Leave room for too large numbers!

  client.publish("home/weather/solarweatherstation/uvindex", _measured_uv_index, 1);      // ,1 = retained
  delay(50);
...
void measurementEvent() { 
  ...
  // print on serial monitor
  Serial.print("IR: ");
  Serial.print(measured_ir);
  Serial.print("%; ");

  // Get UV-Index
  measured_uv_index = uv.readUV();
  measured_uv_index /= 100.0;
  // print on serial monitor
  Serial.print("UV: ");
  Serial.print(measured_uv_index);
  Serial.print("%; ");
